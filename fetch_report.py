...
PLAN_MONTHLY_BUDGET = 300        # общий месячный план бюджета, $

FETCH_SINCE = '2026-03-18'
CHUNK_DAYS = 30
API_VERSION = 'v25.0'
...
# Подписчики Instagram — метрика видна в интерфейсе Ads Manager, но Meta НЕ отдаёт
# её через Marketing/Insights API (подтверждено на форуме Supermetrics, октябрь 2025).
# В API её физически нет — оставляем пустым по этой причине, не гадаем.
FOLLOWER_ACTION_TYPES = set()

# Языки кампаний — ищутся как отдельный сегмент через "_" в названии кампании,
# независимо от регистра. Значение справа — как язык будет подписан на дашборде.
# Работаем только с EN и KA (грузинский) — русский язык не используется в проекте.
LANGUAGE_KEYWORDS = {
    'EN': 'EN',
    'ENG': 'EN',
    'KA': 'KA',
...
def parse_language(name):
    for segment in re.split(r'[_\s]+', name or ''):
        upper = segment.upper()
        if upper in LANGUAGE_KEYWORDS:
            return LANGUAGE_KEYWORDS[upper]
    return None
...
# ============================================================
# 2. Кампании — по дням + язык + статус активности
# ============================================================
...
for lang_name in sorted(set(LANGUAGE_KEYWORDS.values())):
    lang_campaign_ids = [c['id'] for c in campaigns if c['language'] == lang_name]
    if lang_campaign_ids:
        reach_by_preset[lang_name] = {key: fetch_reach(since, until, lang_campaign_ids) for key, (since, until) in PRESET_RANGES.items()}
    else:
        reach_by_preset[lang_name] = {key: 0 for key in PRESET_RANGES}
...
report_data = {
    ...
    "plan": {"monthly_budget": PLAN_MONTHLY_BUDGET},
    "languages": sorted(set(LANGUAGE_KEYWORDS.values())),
...
campaigns_without_language = [c['name'] for c in campaigns if not c['language']]
...
if campaigns_without_language:
    print(f"Кампании без определённого языка ({len(campaigns_without_language)}): {campaigns_without_language}")
